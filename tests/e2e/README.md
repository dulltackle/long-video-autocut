...
| `verify_live_deliverables.py` | 只读 `out/` 产物，断言标准交付物齐全、自洽 |
...
| `clips/*.mp4` | 数量 >= 1，文件非空 |
| `subtitles/*.srt` | 数量 >= 1 |
| `拆条报告.md` | 含「非 dry-run 交付包」与 `| `metadata.json` | yes |` 行 |
| 计数一致 | `metadata.clips` 数 == `clips/*.mp4` 数 == `subtitles/*.srt` 数 |

也可单独跑校验器（针对已存在的产物目录）：

```bash
python tests/e2e/verify_live_deliverables.py out
```
