...
def test_clip_segment_uses_input_side_seek_without_subtitles(monkeypatch):
    calls = []
    segment = SimpleNamespace(start_time=2.0, end_time=8.0)
...
def test_clip_segment_rejects_invalid_time_range(monkeypatch):
    ...
    invalid = SimpleNamespace(start_time=8.0, end_time=2.0)
    assert media.clip_segment("input.mov", invalid, "out.mp4") is False

    negative = SimpleNamespace(start_time=-1.0, end_time=2.0)
