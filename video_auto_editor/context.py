...
class CourseContext:
    ...
    def summary(self):
        """返回可打印摘要，不泄露完整上下文内容。"""
        known_present = sorted(field for field in KNOWN_FIELDS if field in self.data)
        list_counts = {
            field: len(self.data[field])
            for field in sorted(STRING_LIST_FIELDS)
            if field in self.data
        }
        string_fields = sorted(field for field in STRING_FIELDS if field in self.data)
        unknown_fields = sorted(field for field in self.data if field not in KNOWN_FIELDS)
        return {
